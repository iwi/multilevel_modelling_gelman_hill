...
# no predictors
ybarbar <- mean(log.radon)

sample.size <- as.vector(table(county))
sample.size.jittered <- sample.size * exp(runif(J, -0.1, 0.1))
...
mlm.radon.nopred <- bugs(radon.data,
                         radon.inits,
                         radon.parameters,
                         n.chains = 3,
                          "radon.multilevel.nopred.bug",
                         bugs.directory = "c:/.../",
...
plot(sample.size.jittered,
     ...
     pch = 20, log = "x", cex = 0.3, mgp=c(1.5, 0.5, 0),
